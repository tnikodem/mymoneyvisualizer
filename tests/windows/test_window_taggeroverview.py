...
def test_add_single_tagger(tmp_path, qtbot, config, window_main_account_full, test_input_df):
    ...
    window_main.tagger_window.tagger_widget.name_textbox.clear()
    qtbot.keyClicks(window_main.tagger_window.tagger_widget.name_textbox, "tagger1")
    qtbot.keyClicks(window_main.tagger_window.tagger_widget.rec_regex_textbox, "rec b")
    qtbot.keyClicks(window_main.tagger_window.tagger_widget.des_regex_textbox, "des 7")
    ...
    sum2 = float(test_input_df.query(f"{nn.recipient} == 'rec b' and {nn.description} == 'des 7'")[nn.value].sum())
...
def test_add_and_delete(tmp_path, qtbot, config, window_main_account_full, test_input_df):
    ...
    for i in range(3):
        ...
        qtbot.mouseClick(window_main.tagger_overview_window.button_add, Qt.MouseButton.LeftButton)
        qtbot.keyClicks(window_main.tagger_window.tagger_widget.rec_regex_textbox, "rec b")
        qtbot.keyClicks(window_main.tagger_window.tagger_widget.des_regex_textbox, "des "+str(i+1))
    ...
    window_main.tagger_window.tagger_widget.name_textbox.clear()
    qtbot.keyClicks(window_main.tagger_window.tagger_widget.name_textbox, "tagger42")
    window_main.tagger_window.tagger_widget.tag_textbox.clear()
    qtbot.keyClicks(window_main.tagger_window.tagger_widget.tag_textbox, "tag42")
    qtbot.mouseClick(window_main.tagger_window.tagger_widget.button_ok, Qt.MouseButton.LeftButton)
    qt_df_taggeroverview = qt_table_to_dataframe(taggeroverview_table)
...
def test_sort_overview(tmp_path, qtbot, config_full):
    config = config_full
    tagger_overview_window = WindowTaggerOverview(parent=None, config=config, tagger_window=None)
    qtbot.add_widget(tagger_overview_window)

    tagger_overview_window.table.table_widget.sortByColumn(2, Qt.SortOrder.DescendingOrder)
    dfs = []
    for acc in config.accounts.get():
        dfs += [acc.df]
    df = pd.concat(dfs, sort=False).sort_values(nn.date, ascending=False).reset_index(drop=True)
    dfg = df.groupby(nn.tag).agg({nn.date: "count", nn.value: "sum"}).sort_values(nn.date, ascending=False)
    qt_df_taggeroverview = qt_table_to_dataframe(tagger_overview_window.table)
    assert list(dfg[nn.date]) == list(qt_df_taggeroverview["count"].astype(int))
