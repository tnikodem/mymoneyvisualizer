...
def test_create_account(tmp_path, qtbot, config):
    ...
    # save account
    qtbot.mouseClick(window_main.accounts_window.tab_widget.tab_widgets[0].savebutton, Qt.MouseButton.LeftButton)
    expected_filepath = default_db_filepath+default_account_name.replace(" ", "_") + ".csv"
...
def test_money_transfer(tmp_path, qtbot, window_main_two_accounts_full_tagged):
    ...
    assert abs(float(window_main.accounts_window.tab_widget.tab_widgets[0].saldo_textbox.text()) + 50.0) < 0.0001

    qt_df_cash = qt_table_to_dataframe(table_cash)
    assert len(qt_df_cash) == 2
    assert set(qt_df_cash[nn.tag].unique()) == {"cash", "grocery"}
    assert abs(float(window_main.accounts_window.tab_widget.tab_widgets[1].saldo_textbox.text()) - 40.0) < 0.0001
